...
- `1px` and `2px` inside borders and `1px` dividers may be used when their parent frame remains on the 4px grid.
...
| Touch target | A `32px` visible control centered in a `48×48px` target; label the `44px` minimum and actual `48px` size | `A small visible control can use a larger invisible interaction target; this example uses 8px insets to stay on the grid.` |
...
Finish with: `The 4px grid supports predictable spacing and sufficiently sized controls.` Report the design checks actually performed and the remaining implementation checks under [Accessibility](accessibility.md).

Link to the complete spacing and shared sizing references in [Spacing](spacing.md). Explain the structural rule and the permitted rendering exceptions with measured examples; keep detailed audit output internal. The guide fails QA if any visual measurement is unlabeled or the correct and incorrect examples lack their written instruction.
